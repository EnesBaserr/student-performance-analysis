...
# Function to process the dataset and train a Gradient Boosting Machine model
def process_and_train_gbm(file_path):
    # Load the dataset
...
# Train and evaluate on all datasets using GBM
original_mse, original_mae, original_r2, original_y_test, original_y_pred = process_and_train_gbm(original_file)
outliers_mse, outliers_mae, outliers_r2, outliers_y_test, outliers_y_pred = process_and_train_gbm(outliers_removed_file)
iqr_mse, iqr_mae, iqr_r2, iqr_y_test, iqr_y_pred = process_and_train_gbm(iqr_file)
