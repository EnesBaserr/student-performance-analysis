...
# Function to process the dataset and train a model
...
original_mse, original_mae, original_r2, original_y_test, original_y_pred = process_and_train(original_file)
outliers_mse, outliers_mae, outliers_r2, outliers_y_test, outliers_y_pred = process_and_train(outliers_removed_file)
iqr_mse, iqr_mae, iqr_r2, iqr_y_test, iqr_y_pred = process_and_train(iqr_file)
...
# results.to_csv("output/compare_metrics.csv", index=False)
...
# Add text labels on top of bars for MSE and MAE like in the R² plot
for bar in bars_mse:
    yval = bar.get_height()
